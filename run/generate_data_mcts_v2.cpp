...
int main() {
    ...
    auto evaluator_ptr = std::make_unique<rl::deeplearning::NetworkEvaluator>(
        std::move(network_ptr),
        n_actions,
        obs_shape
    );

    // 4. Initialize Generator (tree-search-driven, reliable but slower than the raw generator)
    rl::training::MctsNNUEDataGeneratorV2 generator(
        std::move(evaluator_ptr),
        n_actions,
        /*n_concurrent_games=*/128,
        /*n_simulations_per_move=*/800,
        /*max_async_simulations_per_tree=*/8);

    // 5. Run Generation
    int total_samples = 1000000;
    std::string output_path = "training_data_mcts_384.bin";

    std::cout << "Starting MCTS-driven generation (384-feature layout)..." << std::endl;
