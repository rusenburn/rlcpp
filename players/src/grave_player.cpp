...
{
    GravePlayer::GravePlayer(int minimum_simulations,
    ...
    GravePlayer::~GravePlayer() = default;

    int GravePlayer::choose_action(const std::unique_ptr<rl::common::IState> &state_ptr)
    {
        int n_game_actions = state_ptr->get_n_actions();
        auto tree = Grave(n_game_actions, min_ref_count_, b_squared_, save_illegal_amaf_actions_);
