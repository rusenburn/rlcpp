...
{
RandomEvaluator::RandomEvaluator(int n_game_actions)
    : n_game_actions_{ n_game_actions }
{
}

RandomEvaluator::~RandomEvaluator() = default;

std::tuple<std::vector<float>, std::vector<float>> RandomEvaluator::evaluate(const std::vector<const rl::common::IState*>& state_ptrs)
{
...
std::tuple<std::vector<float>, std::vector<float>> RandomEvaluator::evaluate(const rl::common::IState* state_ptrs)
{
    std::vector<float> probs{};
    std::vector<float> values{};
...
std::tuple<std::vector<float>, std::vector<float>> RandomEvaluator::evaluate(const std::unique_ptr<rl::common::IState>& state_ptrs)
{
    std::vector<float> probs{};
    std::vector<float> values{};
    probs.reserve(n_game_actions_);
...
void RandomEvaluator::evaluate(const rl::common::IState* state_ptr, std::vector<float>& probs, std::vector<float>& values) const
{
    ...
    auto value = rl::common::get() * 2.0 - 1;
    values.emplace_back(value);
}

int RandomEvaluator::choose_action(const std::vector<bool>& masks) const
{
...
std::unique_ptr<IEvaluator> RandomEvaluator::clone() const
{
    return std::unique_ptr<RandomEvaluator>(new RandomEvaluator(*this));
}

std::unique_ptr<IEvaluator> RandomEvaluator::copy() const
{
    return std::unique_ptr<RandomEvaluator>(new RandomEvaluator(*this));
}
